 ...
 *     GET    /v1/auth/dev/start?session=  the page the browser opens
 *     POST   /v1/auth/logout
...
function createLoginSession(response, body) {
  ...
  // on someone clicking a button. Opening the URL by hand completes it too.
  if (provider === "dev") {
    setTimeout(() => complete(session, "Dev Player"), DEV_DELAY_MS).unref?.();
  }
...
/** The page the browser lands on. Completing here is what a real OAuth
 *  callback does after the provider sends the player back. */
function devStart(response, rawId) {
  const session = sessions.get(rawId);
  if (!session) return notFound(response);
  if (session.status === "pending") complete(session, "Dev Player");
  ...
  response.end(
    `<!doctype html><meta charset="utf-8"><title>JKNet</title>` +
      ...
      `<h1>Signed in as ${escapeHtml(session.user?.displayName ?? "Dev Player")}</h1>` +
      `<p>You can return to JKNet.</p></body>`,
  );
}
