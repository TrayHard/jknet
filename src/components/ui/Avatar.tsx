...
/** The three states the status ring shows, matching `Presence.status`. */
export type AvatarStatus = "online" | "in_game" | "offline";

interface AvatarProps {
  /** Full name; the initials are taken from it. */
  name: string;
    ...
    <span className={cn("relative inline-flex shrink-0", className)}>
      {src ? (
        <img
          src={src}
          alt=""
          className={cn("rounded-full object-cover bg-elevated", SIZES[size])}
        />
      ) : (
        <span
          aria-hidden="true"
