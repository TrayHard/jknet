...
- **Профиль игрока** — кто игрок внутри игры: никнейм, скин, рукоятки и цвета. Принадлежит клиенту, лежит в `clients\<slug>\profiles.json`. Сущность добавил пользователь 2026-09-11, устройство описывает раздел [«Профили игрока»](docs/architecture.md#профили-игрока).
...
Профиля клиента в JKNet нет. Не вводите обёртку над связкой «движок + файлы + настройки»: это дубль клиента. Слово «профиль» без уточнения не пишите ни в коде, ни в интерфейсе: пишите «профиль игрока».
...
- Не заводите профиль клиента: сущностей четыре — движок, клиент, файл библиотеки, профиль игрока. Профиль игрока — это косметика поверх клиента, а не второй способ описать сам клиент.
